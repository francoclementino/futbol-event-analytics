"""
Módulo para calcular Expected Threat (xT) en partidos de fútbol
...
# ============================================
# Valores basados en datos de la Premier League
# Fuente: Karun Singh - https://karun.in/blog/expected-threat.html
...
]).T  # Transponer para que coincida con orientación de campo

def get_xt_value(x, y, grid_width=12, grid_height=8):
    """
    Obtiene el valor xT para una coordenada específica.
    
    Args:
        x: Coordenada X (0-100)
        y: Coordenada Y (0-100)
        grid_width: Ancho de la grilla (default: 12)
        grid_height: Alto de la grilla (default: 8)
    
    Returns:
        float: Valor xT en esa posición
    """
    # Normalizar coordenadas a la grilla
...
def calculate_pass_xt(start_x, start_y, end_x, end_y):
    ...
    Args:
        ...
        end_x, end_y: Coordenadas de fin
    
    Returns:
        float: xT añadido por el pase
    """
...
def calculate_player_xt(passes_df):
    ...
    Args:
        passes_df: DataFrame con columnas ['player_id', 'x', 'y', 'end_x', 'end_y', 'outcome']
    
    Returns:
        dict: {player_id: xt_total}
    """
    player_xt = {}
    
    for idx, pass_row in passes_df.iterrows():
        if not pass_row['outcome']:  # Solo pases exitosos
            continue
...
def calculate_connection_xt(passes_df, passer_id, receiver_id):
    """
    Calcula el xT total de una conexión específica entre dos jugadores.
    
    Args:
        passes_df: DataFrame de pases
        passer_id: ID del pasador
        receiver_id: ID del receptor
    
    Returns:
        float: xT total acumulado en esa conexión
    """
    # Filtrar pases de esa conexión específica
...
def add_xt_to_passes(passes_list):
    ...
    Args:
        passes_list: Lista de diccionarios de pases
    
    Returns:
        Lista con 'xt' agregado a cada pase
    """
...
def get_xt_color_intensity(xt_value, max_xt):
    """
    Convierte un valor xT a intensidad de color (0-1).
    
    Args:
        xt_value: Valor xT del jugador
        max_xt: Valor xT máximo en el equipo
    
    Returns:
        float: Intensidad (0-1), donde 1 es el máximo
    """
    if max_xt == 0:
        return 0.5
